...
public class BooksAndAuthors {
  ...
  public static void main(String[] args) throws Exception {
    ...
    DataSet<Tuple2<Integer, String>> authors =
        env.readCsvFile(Config.inputPathTo("authors.tsv"))
           .fieldDelimiter('\t')
           .types(Integer.class, String.class);

    DataSet<Tuple3<Integer, Short, String>> books =
        env.readCsvFile(Config.inputPathTo("books.tsv"))
           .fieldDelimiter('\t')
           .types(Integer.class, Short.class, String.class);

    // IMPLEMENT THIS STEP
      DataSet<Tuple2<String, String>> bookAndAuthor =
              books.join(authors).where(BOOK_AUTHORID_FIELD).equalTo(AUTHOR_ID_FIELD).projectSecond(AUTHOR_NAME_FIELD).projectFirst(BOOK_NAME_FIELD);
